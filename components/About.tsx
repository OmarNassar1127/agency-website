...
const About = forwardRef<HTMLDivElement, AboutProps>(({ isFullPage = false }, ref) => {
  const { t } = useLanguage();
  ...
  return (
    <section 
      className={`${isFullPage ? '' : 'opacity-0'} transition-opacity duration-1000 ease-in-out
    ...
    >
      <div className="relative bg-gradient-to-br from-gray-950 via-primary-900 to-secondary-900 py-24 text-white overflow-hidden">
        ...
        <div className="absolute inset-0 overflow-hidden">
          <div className={`absolute top-1/4 left-1/4 w-8 h-8 bg-white opacity-20 rounded-full ${hasAnimated ? 'animate-float-slow' : ''}`}></div>
          <div className={`absolute top-3/4 left-1/3 w-6 h-6 bg-primary-400 opacity-20 rounded-full ${hasAnimated ? 'animate-float-medium' : ''}`}></div>
          <div className={`absolute top-1/2 right-1/4 w-10 h-10 bg-secondary-400 opacity-20 rounded-full ${hasAnimated ? 'animate-float-fast' : ''}`}></div>
          <div className={`absolute bottom-1/4 right-1/3 w-5 h-5 bg-white opacity-20 rounded-full ${hasAnimated ? 'animate-float-slow' : ''}`}></div>

          {/* Code-like animated elements - only animate when visible */}
          <div className={`absolute top-20 left-10 w-[200px] h-6 bg-white/5 rounded overflow-hidden ${hasAnimated ? 'animate-slide-right' : ''}`}>
          ...
          <div className={`absolute top-1/3 right-10 w-[150px] h-6 bg-white/5 rounded overflow-hidden ${hasAnimated ? 'animate-slide-left' : ''}`}>
            <div className="absolute inset-0 flex items-center px-3">
              <span className="text-xs font-mono text-white/30">
                .transform(ideas)
              </span>
          ...
          <div className={`absolute bottom-1/3 left-20 w-[180px] h-6 bg-white/5 rounded overflow-hidden ${hasAnimated ? 'animate-slide-right' : ''}`}>
            <div className="absolute inset-0 flex items-center px-3">
              <span className="text-xs font-mono text-white/30">
                innovation.unleash()
              </span>
          ...
          <div className={`absolute bottom-1/4 right-20 w-[120px] h-6 bg-white/5 rounded overflow-hidden ${hasAnimated ? 'animate-slide-left' : ''}`}>
            <div className="absolute inset-0 flex items-center px-3">
              <span className="text-xs font-mono text-white/30">
                &lt;/div&gt;
              </span>
        ...
        <div className="container relative z-10 mx-auto px-4 text-center">
          <div className="inline-flex items-center mb-6 px-3 py-1.5 border border-white/10 rounded-full bg-white/5 backdrop-blur-sm">
            <span className={`inline-block w-2 h-2 rounded-full bg-primary-400 mr-2 ${hasAnimated ? 'animate-pulse-slow' : ''}`}></span>
          ...
          <h1 className="text-4xl md:text-5xl lg:text-6xl font-display font-bold mb-6">
            ...
            <span className={`text-transparent bg-clip-text bg-gradient-to-r from-primary-400 to-secondary-400 ${hasAnimated ? 'animate-gradient-x' : ''}`}>
          ...
          <div className="flex flex-wrap justify-center mt-12 relative">
            <div className="relative overflow-hidden rounded-xl border border-white/10 bg-white/5 backdrop-blur p-6 max-w-3xl mx-auto">
              ...
              <div className={`relative pl-4 border-l-2 border-primary-400 ${hasAnimated ? 'typing-animation' : ''}`}>
